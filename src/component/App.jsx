...
function App() {
  ...
  const [sebet, sebetDispatch] = useReducer(
    sebetReducer,
    sessionStorage["basket"] ? JSON.parse(sessionStorage["basket"]) : []
  );
  const [show, setShow] = useState(false);
  const [filter, setFilter] = useState("All");
  const [search, setSearch] = useState("");
  const filterResult = (array) =>
    filter == "All"
      ? array
  ...
  useEffect(() => {
    sessionStorage.setItem("basket", JSON.stringify(sebet));
  }, [sebet]);
